...
drelease:
	...
	@echo "---Build docker---"
	@(cd $(CURDIR) && docker build -t madnikulin50/pnp-lowcode:2026.6.22 .)
	@echo "---Push docker---"
	@(cd $(CURDIR) && docker push madnikulin50/pnp-lowcode:2026.6.22)
...
ddebug:
	...
	##@(cd $(CURDIR)/client && make build) || true
	@echo "---Build docker---"
	@(cd $(CURDIR) && docker build -t pnp-lowcode:2026.6.22 .)
