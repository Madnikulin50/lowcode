...
drelease:
	...
	@echo "---Build docker---"
	@(cd $(CURDIR) && docker build -t madnikulin50/pnp-lowcode:2026.8.13 .)
	@echo "---Push docker---"
	@(cd $(CURDIR) && docker push madnikulin50/pnp-lowcode:2026.8.13)
...
ddebug:
	...
	@(cd $(CURDIR)/client3 && make build) || true
	@echo "---Build docker---"
	@(cd $(CURDIR) && docker build -t pnp-lowcode:2026.8.13 .)
...
dncdebug:
	...
	@(cd $(CURDIR)/client3 && make build) || true
	@echo "---Build docker---"
	@(cd $(CURDIR) && docker build --no-cache -t pnp-lowcode:2026.8.13 .)
