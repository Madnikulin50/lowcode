...
drelease:
	...
	@echo "---Build docker---"
	@(cd $(CURDIR) && docker build -t madnikulin50/pnp-lowcode:2026.7.5-2 .)
	@echo "---Push docker---"
	@(cd $(CURDIR) && docker push madnikulin50/pnp-lowcode:2026.7.5-2)
...
ddebug:
	...
	##@(cd $(CURDIR)/client && make build) || true
	@echo "---Build docker---"
	@(cd $(CURDIR) && docker build -t pnp-lowcode:2026.7.5-2 .)
