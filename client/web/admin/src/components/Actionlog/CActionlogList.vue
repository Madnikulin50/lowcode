      ...
      <form class="d-flex flex-column w-100" @submit.prevent="search">
        <div class="row">
          <div class="col-12 col-lg-6 mb-2">
            ...
            <c-input-date-time v-model="filter.from" :labels="{ clear: $t('general:label.clear'), none: $t('general:label.none'), now: $t('general:label.now'), today: $t('general:label.today') }" />
          </div>
          <div class="col-12 col-lg-6 mb-2">
            <label class="text-primary small">{{ $t('filter.to') }}</label>
            <c-input-date-time v-model="filter.to" only-past :labels="{ clear: $t('general:label.clear'), none: $t('general:label.none'), now: $t('general:label.now'), today: $t('general:label.today') }" />
          </div>
        </div>
        <div class="row">
    ...
    <div class="table-responsive">
      <table class="table table-hover mb-0 small">
        ...
        <tbody v-if="!processing">
          <tr v-for="a in items" :key="a.actionID" class="pointer" @click="a._showDetails = !a._showDetails">
            <td class="text-nowrap">{{ a.timestamp | locFullDateTime }}</td>
            <td>
          ...
          <tr v-if="a._showDetails" v-for="a in items" :key="'detail-' + a.actionID">
            <td colspan="7">
              <div class="row">
                <div class="col-6">
