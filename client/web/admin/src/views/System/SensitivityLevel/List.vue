...
const fields = computed(() => [{ key: 'meta.name', label: t('list.columns.meta.name'), accessor: (i) => i.meta?.name }, { key: 'level', label: t('list.columns.level') }, { key: 'createdAt', label: t('list.columns.createdAt'), formatter: (v) => moment(v).fromNow() }, { key: 'actions', class: 'actions', label: '' }])
