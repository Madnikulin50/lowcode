<template>
  <div class="container-fluid d-flex flex-column flex-fill pt-2 pb-3">
    ...
    >
      <template #header>
        <button v-if="canCreate" class="btn btn-primary btn-lg" @click="$router.push({ name: 'system.role.new' })">{{ $t('new') }}</button>
        <c-permissions-button v-if="canGrant" resource="corteza::system:role/*" :button-label="$t('permissions')" size="lg" />
      ...
      <template #toolbar>
        <c-resource-list-status-filter v-model="filter.deleted" data-test-id="filter-deleted-roles" :label="$t('deleted.label')" :excluded-label="$t('excluded.label')" :inclusive-label="$t('inclusive.label')" :exclusive-label="$t('exclusive.label')" @change="filterList" />
        <c-resource-list-status-filter v-model="filter.archived" data-test-id="filter-archived-roles" :label="$t('archived.label')" :excluded-label="$t('excluded.label')" :inclusive-label="$t('inclusive.label')" :exclusive-label="$t('exclusive.label')" @change="filterList" />
        <div class="col" />
      </template>
      <template #actions="{ item: r }">
        <div v-if="(areActionsVisible({ resource: r, conditions: ['canDeleteRole', 'canGrant'] }) && r.roleID)" class="dropdown">
          ...
          <ul class="dropdown-menu m-0">
            <li><c-permissions-button v-if="canGrant" :title="r.name || r.handle || r.roleID" :target="r.name || r.handle || r.roleID" :resource="`corteza::system:role/${r.roleID}`" :button-label="$t('permissions')" class="dropdown-item" /></li>
