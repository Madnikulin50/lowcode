...
## Next Move
- (задача «Заголовок Dashboard» завершена; «стековый bar chart» завершена; «Magnify» завершена; «Экспорт чата в DOCX» завершена — красивая вёрстка работает, CDP-проверена). При желании: добить EN-лейблы в админке (переводы в `locale/{en,ru}/corteza-webapp-admin/system.users.yaml` — «Профиль avatar» это `editor.photo`? — проверить ключи и scope), либо новая задача
