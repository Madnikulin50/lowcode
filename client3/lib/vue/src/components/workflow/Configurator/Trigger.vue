  ...
  <div>
    ...
    <div v-if="showConstraints" class="card flex-grow-1 border-bottom border-light rounded-0">
      ...
      <div class="card-body p-0">
        ...
        <template v-for="(c, index) in item.triggers.constraints" :key="'detail-' + index">
        <div v-if="c._showDetails" class="mb-3 px-3">
...
onMounted(async () => {
  ...
  props.item.triggers.constraints.forEach(c => { c._showDetails = c._showDetails ?? false })
  await getEventTypes()
})
