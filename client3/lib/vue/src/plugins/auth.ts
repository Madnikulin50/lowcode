...
export default function (opts?: Partial<AuthCtor & { rootApp: boolean }>): Plugin {
  return {
    install(app: App): void {
      ...
      if (!cortezaAuthURL) {
        const { CortezaAPI = undefined, CortezaAuth = undefined } = window as Record<string, string | undefined>
      ...
      if (!callbackURL) {
        ...
        const { CortezaWebapp = undefined } = window as Record<string, string | undefined>
        ...
        if (!rootApp) {
          callbackURL = callbackURL!.replace(/\/$/, '') + `/${_app}`
        }
