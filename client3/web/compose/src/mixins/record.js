...
export default {
  ...
  computed: {
    getModuleByID () {
      return (moduleID) => useModuleStore().getByID(moduleID)
    },
      ...
      this.blocks.forEach((b, index) => {
        if (b.kind === 'RecordList' && b.options.editable) {
          const p = new Promise((resolve) => {
            ...
            emitEvent(`record-line:collect:${recordListUniqueID}`, resolve)
          })
      ...
      return this
        ...
        .then(() => {
          ...
          if (record.valueErrors.set) {
            ...
            this.setWarnings()
            emitEvent('refetch-records', { recordID: record.recordID })
    ...
    handleBulkUpdateSelectedRecords: throttle(function (query) {
      ...
      return this
        ...
        .then(() => {
          ...
          emitEvent('refetch-records', { stayOnPage: true })
