...
export default {
  ...
  computed: {
    getPageLayouts () {
      return (pageID) => usePageLayoutStore().getByPageID(pageID)
    },

    layoutRequiredFields () {
      return () => useUiStore().layoutRequiredFields
    },
  ...
  methods: {
    clearRecordSet () {
      return useRecordStore().clearSet()
    },

    updatePageSet (v) {
      return usePageStore().updateSet(v)
    },

    popPreviousPages () {
      return useUiStore().popPreviousPages()
    },

    popModalPreviousPage () {
      return useUiStore().popModalPreviousPage()
    },
