<template>
  <div class="container-fluid d-flex flex-column flex-fill pt-2 pb-3">
    ...
    <c-resource-list :primary-key="primaryKey" :filter="filter" :sorting="sorting" :pagination="pagination" :fields="fields" :items="items" :row-class="genericRowClass" :translations="{ searchPlaceholder: $t('system.templates.list.filterForm.handle.placeholder'), notFound: $t('admin.general.notFound'), noItems: $t('general.resource-list.no-items'), loading: $t('system.templates.list.loading'), showingPagination: 'general.pagination.showing', singlePluralPagination: 'general.pagination.single', prevPagination: $t('admin.general.pagination.prev'), nextPagination: $t('admin.general.pagination.next'), resourceSingle: $t('label.template.single'), resourcePlural: $t('label.template.plural') }" clickable sticky-header class="custom-resource-list-height flex-fill" @search="filterList" @row-clicked="handleRowClicked">
      <template #header>
        <button v-if="canCreate" class="btn btn-primary btn-lg" @click="$router.push({ name: 'system.template.new' })">{{ $t('system.templates.list.new') }}</button>
        <c-permissions-button v-if="canGrant" resource="corteza::system:template/*" :button-label="$t('system.templates.list.permissions')" size="lg" />
      ...
      <template #toolbar>
        <c-resource-list-status-filter v-model="filter.deleted" data-test-id="filter-deleted-template" :label="$t('system.templates.list.filterForm.deleted.label')" :excluded-label="$t('system.templates.list.filterForm.excluded.label')" :inclusive-label="$t('system.templates.list.filterForm.inclusive.label')" :exclusive-label="$t('system.templates.list.filterForm.exclusive.label')" @change="filterList" />
      </template>
      <template #actions="{ item: t }">
        <div v-if="(areActionsVisible({ resource: t, conditions: ['canDeleteTemplate', 'canGrant'] }) && t.templateID)" class="dropdown">
          ...
          <ul class="dropdown-menu m-0">
            <li><c-permissions-button v-if="canGrant" :title="t.meta.short || t.handle || t.templateID" :target="t.meta.short || t.handle || t.templateID" :resource="`corteza::system:template/${t.templateID}`" :button-label="$t('system.templates.list.permissions')" class="dropdown-item" /></li>
...
<script setup>
defineOptions({ i18nOptions: { namespaces: 'system.templates', keyPrefix: 'list' } })
...
const fields = computed(() => [{ key: 'meta.short', label: t('columns.meta.short'), accessor: (i) => i.meta?.short }, { key: 'handle', sortable: true, label: t('columns.handle') }, { key: 'createdAt', sortable: true, label: t('columns.createdAt'), formatter: (v) => moment(v).fromNow() }, { key: 'actions', class: 'actions', label: '' }])
...
function getActionText(r) { return r.deletedAt ? t('system.templates.list.undelete') : t('system.templates.list.delete') }
