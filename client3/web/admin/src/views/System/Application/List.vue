<template>
  <div class="container-fluid d-flex flex-column flex-fill pt-2 pb-3">
    ...
    <c-resource-list
      ...
      :translations="{ searchPlaceholder: $t('system.applications.list.filterForm.query.placeholder'), notFound: $t('admin.general.notFound'), noItems: $t('general.resource-list.no-items'), loading: $t('system.applications.list.loading'), showingPagination: 'general.pagination.showing', singlePluralPagination: 'general.pagination.single', prevPagination: $t('admin.general.pagination.prev'), nextPagination: $t('admin.general.pagination.next'), resourceSingle: $t('label.application.single'), resourcePlural: $t('label.application.plural') }"
      clickable sticky-header class="custom-resource-list-height flex-fill" @search="filterList" @row-clicked="handleRowClicked"
    >
      <template #header>
        <button v-if="canCreate" class="btn btn-primary btn-lg" @click="$router.push({ name: 'system.application.new' })">{{ $t('system.applications.list.new') }}</button>
        <c-permissions-button v-if="canGrant" resource="corteza::system:application/*" :button-label="$t('system.applications.list.permissions')" size="lg" />
      </template>
      <template #toolbar>
        <c-resource-list-status-filter v-model="filter.deleted" data-test-id="filter-deleted-apps" :label="$t('system.applications.list.filterForm.deleted.label')" :excluded-label="$t('system.applications.list.filterForm.excluded.label')" :inclusive-label="$t('system.applications.list.filterForm.inclusive.label')" :exclusive-label="$t('system.applications.list.filterForm.exclusive.label')" @change="filterList" />
      </template>
      <template #actions="{ item: a }">
        <div v-if="(areActionsVisible({ resource: a, conditions: ['canDeleteApplication', 'canGrant'] }) && a.applicationID)" class="dropdown">
          ...
          <ul class="dropdown-menu m-0">
            <li><c-permissions-button v-if="canGrant" :title="a.name || a.applicationID" :target="a.name || a.applicationID" :resource="`corteza::system:application/${a.applicationID}`" :button-label="$t('system.applications.list.permissions')" class="dropdown-item" /></li>
...
<script setup>
defineOptions({ i18nOptions: { namespaces: 'system.applications', keyPrefix: 'list' } })
...
const fields = computed(() => [{ key: 'name', sortable: true, label: t('columns.name') }, { key: 'unify.name', label: t('columns.appListName'), accessor: (i) => i.unify?.name }, { key: 'enabled', label: t('columns.enabled'), formatter: (v) => v ? 'Yes' : 'No' }, { key: 'createdAt', sortable: true, label: t('columns.createdAt'), formatter: (v) => moment(v).fromNow() }, { key: 'actions', label: '', class: 'actions' }])
...
function getActionText(r) { return r.deletedAt ? t('system.applications.list.undelete') : t('system.applications.list.delete') }
