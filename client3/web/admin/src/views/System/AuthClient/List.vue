      ...
      <template #actions="{ item: a }">
        <div v-if="(areActionsVisible({ resource: a, conditions: ['canDeleteAuthClient', 'canGrant'] }) && a.authClientID)" class="dropdown">
          ...
          <ul class="dropdown-menu m-0">
            <li><c-permissions-button v-if="canGrant" :title="a.meta.name || a.handle || a.authClientID" :target="a.meta.name || a.handle || a.authClientID" :resource="`corteza::system:auth-client/${a.authClientID}`" :button-label="$t('permissions')" class="dropdown-item" /></li>
...
const fields = computed(() => [{ key: 'meta.name', sortable: false, label: t('list.columns.meta.name'), accessor: (i) => i.meta?.name }, { key: 'handle', sortable: true, label: t('list.columns.handle') }, { key: 'enabled', label: t('list.columns.enabled'), formatter: (v) => v ? 'Yes' : 'No' }, { key: 'createdAt', sortable: true, label: t('list.columns.createdAt'), formatter: (v) => moment(v).fromNow() }, { key: 'actions', class: 'actions', label: '' }])
