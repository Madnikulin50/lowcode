<template>
  <div class="container pt-2 pb-3">
    <c-content-header :title="title">
      <button v-if="nodeID" class="btn btn-link" @click="generate.modal = true">{{ $t('federation.nodes.editor.generateUri') }}</button>
    </c-content-header>
      ...
      <div class="modal-dialog modal-lg modal-dialog-centered">
        <div class="modal-content px-5">
          <div class="modal-body text-center px-5">
            ...
            <h2>{{ $t('federation.nodes.editor.generate.description') }}</h2>
            <div class="input-group input-group-lg mt-5">
              <input v-model="generate.email" type="email" class="form-control" placeholder="email@example.com" />
              <button class="btn btn-primary" :disabled="!generate.url || !generate.email" @click="sendEmail()">{{ generate.processing ? $t('federation.nodes.list.loading') : generate.success ? '✓' : $t('federation.nodes.editor.generate.sendEmail') }}</button>
            </div>
            <div class="mt-3">
              <p>{{ $t('federation.nodes.editor.generate.subject') }} <strong>{{ $t('federation.nodes.editor.generate.invitation') }}</strong></p>
              <p class="mt-4">{{ $t('federation.nodes.editor.generate.hello') }}</p>
              <p>{{ $t('generate.body', { userLabel }) }}</p>
              <p class="text-center text-break"><i>{{ generate.url || $t('federation.nodes.editor.generate.notGenerated') }}</i></p>
              <p>{{ $t('federation.nodes.editor.generate.kindRegards') }}</p>
            </div>
            <hr class="my-3" />
            <span class="text-break">{{ generate.url || $t('federation.nodes.editor.generate.notGenerated') }}</span>
          </div>
        </div>
      </div>
    </div>
...
const title = computed(() => props.nodeID ? t('federation.nodes.editor.title.edit') : t('federation.nodes.editor.title.create'))
...
async function sendEmail() { generate.processing = true; const html = `<p class="mt-4">Hello,</p><p>${userLabel.value} is sending you an invitation for LowCoooode Federated Network.</p><p>To start sharing data between organizations, go to the admin panel of your LowCoooode application, click on &ldquo;Federation&rdquo; and select &ldquo;Pair Federation Network&rdquo; on top right corner.<br />Copy the link below and await confirmation from another administrator.</p><blockquote><p class="text-center text-break"><em>${generate.url}</em></p></blockquote><p>Kind regards, LowCoooode team.</p>`; await window.__ComposeAPI.notificationEmailSend({ to: [generate.email], subject: t('federation.nodes.editor.generate.invitation'), content: { html } }).then(() => { generate.email = ''; generate.success = true; setTimeout(() => { generate.success = false }, 2000) }).finally(() => { generate.processing = false }) }
</script>
