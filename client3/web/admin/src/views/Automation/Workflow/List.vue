<template>
  <div class="container-fluid d-flex flex-column flex-fill pt-2 pb-3">
    ...
    <c-resource-list :primary-key="primaryKey" :filter="filter" :sorting="sorting" :pagination="pagination" :fields="fields" :items="items" :row-class="genericRowClass" :translations="{ searchPlaceholder: $t('query.placeholder'), notFound: $t('admin.general.notFound'), noItems: $t('general.resource-list.no-items'), loading: $t('loading'), showingPagination: 'general.pagination.showing', singlePluralPagination: 'general.pagination.single', prevPagination: $t('admin.general.pagination.prev'), nextPagination: $t('admin.general.pagination.next'), resourceSingle: $t('label.workflow.single'), resourcePlural: $t('label.workflow.plural') }" clickable sticky-header class="custom-resource-list-height flex-fill" @search="filterList" @row-clicked="handleRowClicked">
      <template #header>
        <button v-if="canCreate" class="btn btn-primary btn-lg" @click="$router.push({ name: 'automation.workflow.new' })">{{ $t('new') }}</button>
        <c-permissions-button v-if="canGrant" resource="corteza::automation:workflow/*" :button-label="$t('permissions')" size="lg" />
      </template>
      <template #toolbar>
        <c-resource-list-status-filter v-model="filter.deleted" :label="$t('deleted.label')" :excluded-label="$t('excluded.label')" :inclusive-label="$t('inclusive.label')" :exclusive-label="$t('exclusive.label')" @change="filterList" />
      </template>
      <template #actions="{ item: w }">
        <div v-if="(areActionsVisible({ resource: w, conditions: ['canGrant', 'canDeleteWorkflow'] }) && w.workflowID)" class="dropdown">
          ...
          <ul class="dropdown-menu m-0">
            <li><c-permissions-button v-if="canGrant" :title="w.meta.name || w.handle || w.workflowID" :target="w.meta.name || w.handle || w.workflowID" :resource="`corteza::automation:workflow/${w.workflowID}`" :button-label="$t('permissions')" class="dropdown-item" /></li>
...
const fields = computed(() => [{ key: 'handle', sortable: true }, { key: 'meta.name', label: t('list.columns.name'), accessor: (i) => i.meta?.name }, { key: 'enabled', sortable: true, formatter: (v) => v ? 'Yes' : 'No' }, { key: 'createdAt', sortable: true, formatter: (v) => moment(v).fromNow() }, { key: 'actions', class: 'actions', label: '' }])
