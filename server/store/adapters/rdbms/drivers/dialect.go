...
func OpHandlerEq(d Dialect, n *ql.ASTNode, args ...exp.Expression) (exp.Expression, error) {
	return opHandlerEqNe(d, n, false, args...)
}

func OpHandlerNe(d Dialect, n *ql.ASTNode, args ...exp.Expression) (exp.Expression, error) {
	return opHandlerEqNe(d, n, true, args...)
}

// opHandlerEqNe compares JSON-stored ID/Ref attributes as text.
//
// The default AttributeCast for TypeID/TypeRef wraps the JSON extract in
// `CASE WHEN val ~ '^[0-9]+$' THEN val END::BIGINT`. That predicate cannot
// use an index and is evaluated for every compose_record row of the module.
// Record/User filters are always ID equality (`device = '123'`), so a plain
// text compare of `values->'field'->>0` is equivalent and much cheaper.
func opHandlerEqNe(d Dialect, n *ql.ASTNode, ne bool, args ...exp.Expression) (exp.Expression, error) {
	if left, ok, err := jsonStoredRefText(d, n); err != nil {
		return nil, err
		...
		return exp.NewBooleanExpression(op, left, exp.NewCastExpression(args[1], "TEXT")), nil
	}

	if len(args) < 2 {
		return nil, fmt.Errorf("eq/ne requires 2 arguments")
	}
	if ne {
		return exp.NewBooleanExpression(exp.NeqOp, args[0], args[1]), nil
	}
	return exp.NewBooleanExpression(exp.EqOp, args[0], args[1]), nil
}

func jsonStoredRefText(d Dialect, n *ql.ASTNode) (exp.Expression, bool, error) {
	...
	left, err := d.JsonExtractUnquote(ident, attr.Ident, 0)
	if err != nil {
		return nil, true, err
	}
