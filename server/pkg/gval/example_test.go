...
func Example() {
	...
	value, err := Evaluate(`"Hello " + name + "!"`, vars)
	if err != nil {
		fmt.Println(err)
	}
...
func ExampleEvaluate() {
	...
	value, err := Evaluate("foo > 0", map[string]interface{}{
		"foo": -1.,
	})
	if err != nil {
		fmt.Println(err)
...
func ExampleEvaluate_nestedParameter() {
	...
	value, err := Evaluate("foo.bar > 0", map[string]interface{}{
...
func ExampleEvaluate_complexAccessor() {
	...
	value, err := Evaluate(`foo["b" + "a" + "r"]`, map[string]interface{}{
		"foo": map[string]interface{}{"bar": -1.},
	})
	if err != nil {
		fmt.Println(err)
...
func ExampleEvaluate_arithmetic() {
	...
	value, err := Evaluate("(requests_made * requests_succeeded / 100) >= 90",
...
func ExampleEvaluate_string() {
	...
	value, err := Evaluate(`http_response_body == "service is ok"`,
		map[string]interface{}{
...
func ExampleEvaluate_float64() {
	...
	value, err := Evaluate("(mem_used / total_mem) * 100",
		map[string]interface{}{
			"total_mem": 1024,
			"mem_used":  512,
		})
...
func ExampleEvaluate_dateComparison() {
	...
	value, err := Evaluate("date(`2014-01-02`) > date(`2014-01-01 23:59:59`)",
		nil,
...
func ExampleEvaluable() {
	eval, err := Full(Constant("maximum_time", 52)).
...
func ExampleEvaluate_strlen() {
	...
	value, err := Evaluate(`strlen("someReallyLongInputString") <= 16`,
		nil,
		Function("strlen", func(args ...interface{}) (interface{}, error) {
...
func ExampleEvaluate_encoding() {
	...
	value, err := Evaluate(`(7 < "47" == true ? "hello world!\n\u263a" : "good bye\n")`+" + ` more text`",
		nil,
		Function("strlen", func(args ...interface{}) (interface{}, error) {
			length := len(args[0].(string))
			return (float64)(length), nil
...
func ExampleEvaluate_accessor() {
	...
	value, err := Evaluate(`foo.Hello + foo.World()`,
		map[string]interface{}{
...
func ExampleEvaluate_flatAccessor() {
	...
	value, err := Evaluate(`Hello + World()`,
...
func ExampleEvaluate_nestedAccessor() {
	...
	value, err := Evaluate(`foo.Bar.Hello + foo.Bar.World()`,
		map[string]interface{}{
...
func ExampleVariableSelector() {
	value, err := Evaluate(`hello.world`,
		"!",
		VariableSelector(func(path Evaluables) Evaluable {
...
func ExampleEvaluable_EvalInt() {
	eval, err := Full().NewEvaluable("1 + x")
	if err != nil {
		fmt.Println(err)
		return
	}
...
func ExampleEvaluate_jsonpath() {
	...
	value, err := Evaluate(`$["response-time"]`,
		map[string]interface{}{
			"response-time": 100,
		},
		jsonpath.Language(),
...
func ExampleLanguage() {
	lang := NewLanguage(JSON(), Arithmetic(),
		//pipe operator
		PostfixOperator("|", func(c context.Context, p *Parser, pre Evaluable) (Evaluable, error) {
...
var _ Selector = &exampleCustomSelector{}
...
func parseSub(ctx context.Context, p *Parser) (Evaluable, error) {
	return p.ParseSublanguage(ctx, subLang)
}

var (
	superLang = NewLanguage(
		PrefixExtension('$', parseSub),
	)
	subLang = NewLanguage(
		Init(func(ctx context.Context, p *Parser) (Evaluable, error) { return p.Const("hello world"), nil }),
	)
)
